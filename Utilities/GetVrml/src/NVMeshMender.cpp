...
namespace
{
    ...
    // IC: Replacement for D3DXVec3Normalize so that we don't have dll dependencies.
    static D3DXVECTOR3 * Vec3Normalize( D3DXVECTOR3 *pOut, CONST D3DXVECTOR3 *pV )
    {
        assert(pOut != NULL);
        assert(pV != NULL);

        float sum_of_squares = pV->x * pV->x + pV->y * pV->y + pV->z * pV->z;
        if(sum_of_squares == 0 ) sum_of_squares = 1; // will result in null vector being passed back
        float v = 1.0f / sqrtf(sum_of_squares);
        pOut->x = pV->x * v;
        pOut->y = pV->y * v;
        pOut->z = pV->z * v;
        return pOut;
...
public:
    virtual bool CanSmooth(MeshMender::Triangle* t1, MeshMender::Triangle* t2, const float& minCreaseAngle)
    {
        ...
        float dp = D3DXVec3Dot( &tmp1, &tmp2 );
        dp = MAX(dp, -1.0); // rounding errors can result in -1.00001
        if(dp >= minCreaseAngle )
        {
            return true;
        }
...
public:
    virtual bool CanSmooth(MeshMender::Triangle* t1, MeshMender::Triangle* t2, const float& minCreaseAngle)
    {
        ...
        // ncsoft added, in addition to crease angle, we require that handedness of 
        //    tangent space is the same for each tri, otherwise this is a uv mirror edge
        //    and should not be smoothed.
        if( t1->bRightHanded != t2->bRightHanded )
            return false;
                                  ...
                                  const NormalCalcOption computeNormals)
{
    ...
    {
        // ncsoft fpe 3/5/2009 -- added this section so that normals aren't bogus for later use
        // Use the normals that were passed in, average vert normals to get face normal
        D3DXVECTOR3 faceNormal = verts[t.indices[0]].normal;
        faceNormal += verts[t.indices[1]].normal;
        faceNormal += verts[t.indices[2]].normal;
        Vec3Normalize(&t.normal, &faceNormal);

#if 0
        // fpe for test, make sure this matches sign of geometry face normal calculation above
        D3DXVECTOR3 edge0 = verts[t.indices[1]].pos - verts[t.indices[0]].pos;
        D3DXVECTOR3 edge1 = verts[t.indices[2]].pos - verts[t.indices[0]].pos;
        D3DXVECTOR3 defaultNormal;
        D3DXVec3Cross( &defaultNormal, &edge0, &edge1);
        Vec3Normalize(&defaultNormal,&defaultNormal);

        float dp = D3DXVec3Dot(&defaultNormal, &t.normal);
        if(dp < 0 ) {
            char buf[128];
            sprintf(buf, "got dp less than zero %.3f\n", dp);
            OutputDebugStr(buf);
            //assert(dp > 0); // better be in same direction
        }
#endif
    }
    ...
    // ncsoft added 3/5/2009 - calculate handedness of tangent space
    D3DXVECTOR3 testBiNormal;
    D3DXVec3Cross( &testBiNormal, &t.normal, &t.tangent);
    t.bRightHanded = (D3DXVec3Dot(&testBiNormal , &t.binormal) < 0);
}
    ...
    std::vector< Vertex >&   theVerts )
{
    ...
    for(size_t i = 0 ; i < len ; ++ i )
    {
        ...
        if( (lenTan <= 0.0001f) || (lenBin <= 0.0001f)  ) //should be approx 1.0f
        {    
...
#define MAX_VALID_VALUE    50000.f
#define VALIDATE_VECTOR(vec)    assert(vec.x > -MAX_VALID_VALUE && vec.x < MAX_VALID_VALUE); \
    assert(vec.y > -MAX_VALID_VALUE && vec.y < MAX_VALID_VALUE); \
    assert(vec.z > -MAX_VALID_VALUE && vec.z < MAX_VALID_VALUE);
...
#if 0
    // fpe for test
    float sumtan = tangent.x*tangent.x + tangent.y*tangent.y + tangent.z*tangent.z;
    float sumbin = binormal.x*binormal.x + binormal.y*binormal.y + binormal.z*binormal.z;
    if( sumtan < 0.0001f || sumbin < 0.0001f )
    {
        char buf[256];
        sprintf(buf, "Small tan or bin!:  tangent {%.3f, %.3f, %.3f}, binormal {%.3f, %.3f, %.3f}, s1, s2, t1, t2 = %.3f, %.3f, %.3f, %.3f\n",
            tangent.x, tangent.y, tangent.z, binormal.x, binormal.y, binormal.z, s1, s2, t1, t2);
        OutputDebugStr(buf);
    }
#endif

#if 0 // tbd, revisit this -- should be fine if one is invalid but the other is valid since will reconstruct from xp with normal
    VALIDATE_VECTOR(tangent);
    VALIDATE_VECTOR(binormal);
#endif
