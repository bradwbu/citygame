...
int IsUsing64BitWindows(void)
{
    ...
    LPFN_ISWOW64PROCESS fnIsWow64Process = (LPFN_ISWOW64PROCESS)GetProcAddress(LoadLibraryA("kernel32"), "IsWow64Process");
...
void NotifyTransgamingOfLogin(const char* account_name)
{
    if (account_name && IsUsingCider())
    {
        HMODULE ntdllModule = GetModuleHandle(TEXT("ntdll.dll"));
        typedef void (WINAPI *fnTGActivateFunc)(const char*);
        fnTGActivateFunc fnTGActivate;

        if (ntdllModule)
        {
            fnTGActivate = (void (WINAPI *)(const char*))GetProcAddress(ntdllModule,"TGActivate");
            if ( fnTGActivate )
            {
                fnTGActivate(account_name);
            }
        }
    }
}
