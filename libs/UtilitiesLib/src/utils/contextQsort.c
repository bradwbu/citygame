...
static void __cdecl shortsort(char *lo, char *hi, size_t width, const void* context,
                    ...
                    )
{
    ...
    1 + log2i(num), so 30 is sufficient for any array */
                               ...
                               )
{
    ...
    while (hi > lo) {
        ...
        //this extra if statement makes the sort stable, it will also slow it down a bit,
        //but since it is only called on arrays of length 8 or less it shouldn't be a big
        //problem, if it is, simply comment out the if statement
        if (comp(max,hi,context)>0)
            swap(max, hi, width);
