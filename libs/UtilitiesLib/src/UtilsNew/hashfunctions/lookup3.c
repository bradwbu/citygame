...
size_t          length,               /* the length of the key, in U32s */
...
U32 hashlittle( const void *key, size_t length, U32 initval)
{
...
U32 burtle_hashstr( const char *str )
{
    return str?hashlittle(str,strlen(str),0xabcd1234):0; // arbitrary hash.
}
 ...
 */
U32 hashbig( const void *key, size_t length, U32 initval)
{
  U32 a,b,c;
...
//   printf("%.8x                            %.8x                            %.8x\n",
//          hashword((const U32 *)q, (sizeof(q)-1)/4, 13),
//          hashword((const U32 *)q, (sizeof(q)-5)/4, 13),
//          hashword((const U32 *)q, (sizeof(q)-9)/4, 13));
//   p = q;
...
//   printf("\n");
  ...
  if (hashlittle(q, sizeof(q), 47) != i)
      UTASSERT(0 && "hashlittle2 and hashlittle mismatch\n");
  ...
  if (hashword(&len, 1, 47) != i)
      UTERR("hashword2 and hashword mismatch %x %x\n", 
            i, hashword(&len, 1, 47));
  ...
  for (h=0, b=buf+1; h<8; ++h, ++b)
  {
    for (i=0; i<MAXLEN; ++i)
    {
      ...
      ref = hashlittle(b, len, (U32)1);
      *(b+i)=(U8)~0;
      *(b-1)=(U8)~0;
      x = hashlittle(b, len, (U32)1);
      y = hashlittle(b, len, (U32)1);
      if ((ref != x) || (ref != y)) 
      {
          UTERR("alignment error: %.8x %.8x %.8x %d %d\n",ref,x,y,
                h, i);
      }
    }
  }
