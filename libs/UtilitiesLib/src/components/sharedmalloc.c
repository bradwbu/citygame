/*
This is actually a hacked up version of dlmalloc that allocates pages by calling
our SharedMemory function, thus providing a shared heap allocator. The high-level
management of this should be taken care of in SharedHeap.c.
...
DEFAULT_MMAP_THRESHOLD       default: hacked to 2GB, was 256K
...
// #include <windows.h> // hacked: alternate includes at top
#define HAVE_MMAP 1
...
#if HAVE_MMAP
#define DEFAULT_MMAP_THRESHOLD ((size_t)1024U * (size_t)1024U * (size_t)1024U * (size_t)2U) // hacked: allow huge reservations
#else   /* HAVE_MMAP */
...
static void* allocateSharedPage(size_t size) 
{
    ...
    do {
        ...
        ret = sharedMemoryAcquire(&shared_memory, makeSharedPageName(getSharedHeapMemoryManagerPageAllocs()));
        if ( ret == SHAR_Error )
        {
            // Shared memory is full.  We disable shared memory and continue on.  The caller will try to 
            //    allocate from the regular heap.  TBD: We may want to just fatal error here in SERVER mode.
            sharedHeapMemoryManagerLock(); // caller expects it to be locked and will unlock it
            sharedHeapTurnOff( "Failed to acquire shared heap page handle");
            return NULL;
        }

        sharedHeapMemoryManagerLock();
    } while (ret != SHAR_FirstCaller);

    ptr = sharedMemorySetSizeEx(shared_memory, size, 0);
    if (!ptr)
    {
        sharedHeapTurnOff( "Failed to allocate shared heap page size" );
        return NULL;
    }

    incSharedHeapMemoryManagerPageAllocs(shared_memory);

    // Instead of using sharedMemoryLock() everywher, the sharedHeapMM
    // page is protected by sharedHeapMMMutex instead
    sharedMemoryUnlock(shared_memory);
    sharedMemoryUnprotect(shared_memory, PAGE_READWRITE);
    return ptr;
}
...
//  void* ptr = VirtualAlloc(0, size, MEM_RESERVE|MEM_COMMIT, PAGE_READWRITE);
    void* ptr = allocateSharedPage(size);
...
static void internal_malloc_stats(mstate m) {
  ...
  if (!PREACTION(m)) {
    ...
    // hacked: ms crt format specifier for size_t, was truncating to 32 bits on x64
    fprintf(stderr, "max system bytes = %10Iu\n", maxfp);
    fprintf(stderr, "system bytes     = %10Iu\n", fp);
    fprintf(stderr, "in use bytes     = %10Iu\n", used);
