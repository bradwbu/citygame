 ...
 */
int fast_polygon_intersects_cube(int nverts, const real verts[][3],
...
int            ret;
Vec3        sub_pos;
Vec3        pnorm,tvec1,tvec2;

    setVec3(sub_pos,cube_size * 0.5f,cube_size * 0.5f,cube_size * 0.5f);
    addVec3(cube_pos,sub_pos,sub_pos);

    inv_size = 1.f / (cube_size);

    subVec3(v1,sub_pos,verts[0]);
    subVec3(v2,sub_pos,verts[1]);
    subVec3(v3,sub_pos,verts[2]);

    scaleVec3(verts[0],inv_size,verts[0]);
    scaleVec3(verts[1],inv_size,verts[1]);
    scaleVec3(verts[2],inv_size,verts[2]);

    subVec3(v2,v1,tvec1);
    subVec3(v3,v2,tvec2);
    crossVec3(tvec1,tvec2,pnorm);
    normalVec3(pnorm);

    ret = fast_polygon_intersects_cube(3, verts, pnorm, 0, 0);
