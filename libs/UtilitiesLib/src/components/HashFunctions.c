...
/*    if (((uintptr_t)k)&3)
    {
        while (len >= 12)    // unaligned
        {
            a += (k[0] +((ub4)k[1]<<8) +((ub4)k[2]<<16) +((ub4)k[3]<<24))|0x20202020;
            b += (k[4] +((ub4)k[5]<<8) +((ub4)k[6]<<16) +((ub4)k[7]<<24))|0x20202020;
            c += (k[8] +((ub4)k[9]<<8) +((ub4)k[10]<<16)+((ub4)k[11]<<24))|0x20202020;
            mix(a,b,c);
            k += 12; len -= 12;
        }
    }
    else*/
    {
        while (len >= 12)    /* aligned */
        {
            a += (*(ub4 *)(k+0))|0x20202020;
            b += (*(ub4 *)(k+4))|0x20202020;
            c += (*(ub4 *)(k+8))|0x20202020;
            mix(a,b,c);
            k += 12; len -= 12;
        }
    }
    ...
    switch(len)              /* all the case statements fall through */
    {
    case 11: c+=((ub4)(k[10]|0x20)<<24);
    case 10: c+=((ub4)(k[9]|0x20)<<16);
    case 9 : c+=((ub4)(k[8]|0x20)<<8);
        /* the first byte of c is reserved for the length */
    case 8 : b+=((ub4)(k[7]|0x20)<<24);
    case 7 : b+=((ub4)(k[6]|0x20)<<16);
    case 6 : b+=((ub4)(k[5]|0x20)<<8);
    case 5 : b+=(k[4]|0x20);
    case 4 : a+=((ub4)(k[3]|0x20)<<24);
    case 3 : a+=((ub4)(k[2]|0x20)<<16);
    case 2 : a+=((ub4)(k[1]|0x20)<<8);
    case 1 : a+=(k[0]|0x20);
        /* case 0: nothing left to add */
    }
    mix(a,b,c);
...
{
    if ( bCaseSensitive)
        return burtlehash3(pcToHash, (ub4)strlen(pcToHash), DEFAULT_HASH_SEED);
    else // this version of the hash function sets the lowercase bit, so it does not distinguish between the two possibilities
        return burtlehash3StashDefault(pcToHash, (ub4)strlen(pcToHash), DEFAULT_HASH_SEED);
}
...
{
    return burtlehash3((const ub1*)k, length, initval);
}