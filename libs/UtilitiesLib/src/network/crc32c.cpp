...
static const U32 s_crc32c[0x100] = {
...
static inline U32 Crc32c_cpu(U32 crc, const void * data, size_t bytes) {
    ...
    for (term = start + (bytes & ~7); curr != term; ) {
        crc = s_crc32c[(crc ^ *curr++) & 0xff] ^ (crc >> 8);
        crc = s_crc32c[(crc ^ *curr++) & 0xff] ^ (crc >> 8);
        crc = s_crc32c[(crc ^ *curr++) & 0xff] ^ (crc >> 8);
        crc = s_crc32c[(crc ^ *curr++) & 0xff] ^ (crc >> 8);
        crc = s_crc32c[(crc ^ *curr++) & 0xff] ^ (crc >> 8);
    ...
    for (term = start + bytes; curr != term; )
        crc = s_crc32c[(crc ^ *curr++) & 0xff] ^ (crc >> 8);

    return crc;
}

#ifdef COMPILER_SUPPORTS_SSE42
static inline U32 Crc32c_sse42(U32 crc, const void * data, size_t bytes) {

    crc = endianSwapIf(U32, crc, !isBigEndian());

    size_t dwords = bytes / sizeof(U32);
    bytes -= dwords * sizeof(U32);

    // Process dwords first
    const U32 * pdword = (const U32 *)data;
    while (dwords--) {
        crc = _mm_crc32_u32(crc, *pdword++);
    }

    // Process remaining bytes
    const U8 * pbyte = (const U8 *)pdword;
    while (bytes--) {
        crc = _mm_crc32_u8(crc, *pbyte++);
    }

    return endianSwapIf(U32, crc, !isBigEndian());
}
#endif
...
#ifdef COMPILER_SUPPORTS_SSE42
    static const bool supportsSSE42 = g_CPUIDFlags & CPUID_SSE42;
    if(!supportsSSE42) {
#endif

        return Crc32c_cpu(crc, data, bytes);

#ifdef COMPILER_SUPPORTS_SSE42
    } else {
        U32 result = Crc32c_sse42(crc, data, bytes);
        onlydevassert(result == Crc32c_cpu(crc, data, bytes));
        return result;
    }
#endif
}
