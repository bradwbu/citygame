	...
	for (i = 0; (pti[i].name && pti[i].name[0]) || pti[i].type; i++) {
		if (TOK_GET_TYPE(pti[i].type) == TOK_STRUCT_X) {
			if (pti[i].type & TOK_INDIRECT) {
				csvValues(TokenStoreGetPointer(pti, i, data, 0),
					(TokenizerParseInfo*)pti[i].subtable, comma);
			} else {
		...
		if (data) {
			if (TOK_GET_TYPE(pti[i].type) == TOK_STRUCT_X) {
				if (pti[i].type & TOK_EARRAY) {
					node->isarray = true;
					jsonEArray(node, TokenStoreGetPointer(pti, i, data, 0),
						(TokenizerParseInfo*)pti[i].subtable);
				} else if (pti[i].type & TOK_INDIRECT) {
					jsonStruct(node, TokenStoreGetPointer(pti, i, data, 0),
						(TokenizerParseInfo*)pti[i].subtable);
				} else {
