...
void reloadClientMessageStores(int localeID) {
    ...
    for (i = 0; ppchTexWordsFiles && ppchTexWordsFiles[i]; i += 2) {
    ...
    for (i = 0; ppchLoadingTipFiles && ppchLoadingTipFiles[i]; i += 2) {
        writeConsole(OUTPUT_DEBUG, "\t%s", ppchLoadingTipFiles[i]);
    }
