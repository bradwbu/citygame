...
static float drawPowerSetDetails( F32 x, F32 y, F32 z, F32 powersetHt, const BasePowerSet *powerSet, int uiIndex, int mode)
{
    ...
    if (ht[CURRENT_POWER_CUST_MENU][uiIndex] > 0)
    {
        ...
        {
            ywalk += drawPower(&heSelectedPower, powerSet->ppPowers[powerIndex], REGION_CENTER, ywalk, z, scaleFactor, sSelectedPowerSets[CURRENT_POWER_CUST_MENU] == powerSet, mode, &hovered, powerIndex );                    
        }
        clipperPop();
        clipperPop();
...
static void setPetCostumeColors(Entity* pPet, const AttribModTemplate* mod)
{
    ...
    if (!customization || mod->bDoNotTintCostume || isNullOrNone(customization->token) || 
...
static PowerAnimEvent* createSpawnAnimationEvents(PowerAnimEvent* event, Entity* caster, const AttribModTemplate* mod, int spawnIndex, F32 timeOffset) {
    const VillainDef* def = villainFindByName(mod->pchEntityDef);
...
static bool isPreviewMod(const AttribModTemplate* mod, Entity* pSrc) {
    ...
    if(mod->ppchApplicationRequires && combateval_EvalFromBasePower(pSrc, gTargetEnt, sSelectedPowers[CURRENT_POWER_CUST_MENU], mod->ppchApplicationRequires, sSelectedPowers[CURRENT_POWER_CUST_MENU]->pchSourceFile)==0.0f)
        return false;
