...
static void renderGroupDef(Vec3 headshotCameraPos, GroupDef **defs, bool rotate, AtlasTex * bind, int sizeOfPictureX, int sizeOfPictureY, bool lookFromBelow, Vec3 objPosition)
{
    ...
    for (i=0; i<num_defs; i++)
    {
        ...
        groupGetBoundsMinMaxNoVolumes(defs[i],unitmat,min,max);
        for (j=0; j <= 2; j++)
...
static AtlasTex *getImage(const char *groupDefNames, AtlasTex *oldbind, bool rotate, bool lookFromBelow)
{
    ...
    while (str)
    {
        ...
        if (def) 
        {
            if (strstri(str,"baselogo") && !eaSize(&def->def_tex_swaps))
            {
    ...
    renderGroupDef(headshotCameraPos, defs, rotate, tex, sizeOfPictureX, sizeOfPictureY, lookFromBelow, objPosition);
...
AtlasTex* groupThumbnailGet( const char *groupDefNames, bool rotate, bool lookFromBelow, int *not_cached )
{
    ...
    if (!bind || rotate) {
        // Need to create it!
        bind = getImage(groupDefNames, bind, rotate, lookFromBelow);
        if (rotate) {
