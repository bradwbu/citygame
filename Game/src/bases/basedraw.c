...
void drawDetailBox(const Detail *detail,Mat4 mat, TintedGroupDef *box, TintedGroupDef *box_volume)
{
    ...
    if (!detail)
        return;
    baseDetailExtents(detail,mat,min,max);
    ...
    if (detail->bHasVolumeTrigger && box_volume)
    {
        baseDetailVolumeTriggerExtents(detail,mat,min,max);
        for( i = 0; i < 3; i++ ) // prevent annoying z-flicker on coincident surfaces
        {                        // stupid, but works
            min[i] -= .01f;
            max[i] += .01f; 
        }
        drawBoxExtents(mat[3],min,max,box_volume);
    }
}
