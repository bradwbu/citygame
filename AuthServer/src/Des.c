...
static void contract(char *in48, char *out32)                   /* contract f from 48 to 32 bits*/
{
	...
	*c++ = s[0][0xfff & ((i0 << 4) | ((i1 >> 4) & 0x0f))];
	*c++ = s[1][0xfff & ((i1 << 8) | (i2 & 0xff))];
	*c++ = s[2][0xfff & ((i3 << 4) | ((i4 >> 4) & 0x0f))];
...
static void kinit(char *key64bit)       /* initialize key schedule array*/
{
	REGISTER int i, j, l;
	int m;
...
static void endes(char *inblock, char *outblock)                /* encrypt 64-bit inblock       */
{
...
static void dedes(char *inblock, char *outblock)                /* decrypt 64-bit inblock       */
{
	char iters[17][8];                  /* workspace for each iteration */
	char swap[8];                       /* place to interchange L and R */
	REGISTER int i;
...
static void DesMem( void *buf, int mlen, int isencrypting )
{                                       // process the file
	char *b;
