...
static void roomDetailsToGridInternal(BaseRoom *room,FillState *state,RoomDetail **details,int count,int skip_id)
{
    ...
    for( i=(!eaSize(&room->doors)?1:0); i<count; i++)
    {
        ...
        if(room_detail->id == skip_id || !baseDetailExtents(detail,room_detail->mat,min,max)
            || room_detail->info->bDoNotBlock)
        {
            continue;
        }
        ...
        for(z=min[2];z<max[2];z++)
        {
            for(x=min[0];x<max[0];x++)
            {
                if (z < 0 || x < 0 || z >= zsize || x >= xsize)
                    continue;
                if (detail->eSurface == kSurface_Floor)
                {
                    blocks[z*xsize+x].height[0] = max[1];
                }
                if (detail->eSurface == kSurface_Ceiling)
                {
                    blocks[z*xsize+x].height[1] = min[1];
                }
                if (detail->eSurface == kSurface_Wall)
                {
                    blocks[z*xsize+x].height[0] = max[1];
                    blocks[z*xsize+x].height[1] = min[1];
                }
...
int roomDetailReachable(RoomDetail * detail, FillState * state)
{
    ...
    int    x, z, section_req, minpos[3],maxpos[3];
    ...
    copyVec3(det_min,minpos);
    copyVec3(det_max,maxpos);

    markPassable(state,minpos[0]-1,minpos[2]-1,maxpos[0]+1,maxpos[2]+1);
    //printRoom(state);
 
    // look for contiguos 8ft, unless the item is smaller 
    section_req =  MIN( MAX(ABS(maxpos[2]-minpos[2]), ABS(maxpos[0]-minpos[0])), 8 );

    // look for contiguos section
    for(x=minpos[0];x<maxpos[0];x++)
    {
        min_block = getBlock(state,x,minpos[2]-1);
        max_block = getBlock(state,x,maxpos[2]);
    ...
    if( !bFoundSection )
    {
        ...
        {
            min_block = getBlock(state,minpos[0]-1, z);
            max_block = getBlock(state,maxpos[0], z);
...
int roomDetailLegal(BaseRoom *room,RoomDetail *detail, int * error)
{
    ...
    roomDetailsToGridInternal(room,state,&detail,1,-1);
    {
        ...
        baseDetailExtents(detail->info,detail->mat,min,max);
        markUnpassable(state,min[0],min[2],max[0],max[2],0);
    }
...
BaseRoom *getLegalDoorway(Base *base, BaseRoom *room,int block[2],F32 height[2], int neighbor_block[2] )
{
    ...
    if (roomHeightIntersecting(room,block,height,0,0))
        return 0;
    if (roomHeightIntersecting(neighbor,neighbor_block,height,0,0))
        return 0;
    if (!roomDoorwayLegal(room,block))
        return 0;
    if (!roomDoorwayLegal(neighbor,neighbor_block))
        return 0;
    return neighbor;
}
...
int baseConnected(Base *base)
{
    ...
    for(i=0;i<eaSize(&base->rooms);i++)
        base->rooms[i]->tagged = 0;
     roomsConnectedRecur(base,base->rooms[0]);
    for(i=0;i<eaSize(&base->rooms);i++)
    {
        if (!base->rooms[i]->tagged)
            return 0;
    }
    return 1;
}

int roomDoorwayDeleteLegal(int room_id,int block[2])
{
    Base        temp = {0};
    int            connected;
    RoomDoor    door;

    copyVec2(block,door.pos);
    copyRoomsAndDoors(&temp,0);
    doorAddDel(&temp,room_id,&door,0);
    checkAllDoors(&temp,0);
    connected = baseConnected(&temp);
    freeRoomsAndDoors(&temp);
    return connected;
}

int roomDeleteLegal(int room_id)
{
    Base    temp = {0};
    BaseRoom *room;
    int        connected;
    ...
    copyRoomsAndDoors(&temp,0);
    room = baseGetRoom(&temp,room_id);
    eaFindAndRemove(&temp.rooms,room);
    eaDestroyEx(&room->doors,ParserFreeStruct);
    ParserFreeStruct(room);        
    connected = baseConnected(&temp);
    freeRoomsAndDoors(&temp);
    return connected;
}
...
int roomChangeLegal(BaseRoom *room)
{
    Base        temp = {0};
    BaseRoom    *copy;
    int            connected;

    copyRoomsAndDoors(&temp,room);
    copy = baseGetRoom(&temp,room->id);
    deleteRoomDoors(&temp,copy);
    addRoomDoors(&temp,copy,&room->doors);
    connected = baseConnected(&temp);
    freeRoomsAndDoors(&temp);
    ParserFreeStruct(copy);
    return 1;//connected;
}
...
bool addOrChangeRooomIsLegal( Base * base, int room_id, int size[2], int rotation, Vec3 pos, F32 height[2], const RoomTemplate *info, RoomDoor *doors, int door_count )
{
    ...
    ret =  baseConnected(base) && basePlotFit(base, base->plot_width, base->plot_height, NULL, 0);
...
//    FillState    *state = &fill_state;
    ...
    if (!baseConnected(base))
    {
        if (error)
            *error = kBaseErr_NotConnected;
        return 0;
    }
    ...
    //    room = base->rooms[i];
    //    roomBlocksToGrid(room,state,0,0);
    //    roomDetailsToGridInternal(room,state,room->details,eaSize(&room->details),0);
    //    markUnpassable(&fill_state,0,0,fill_state.xsize,fill_state.zsize,1);
    //    if (!roomDoorsConnect(room,state,0))
    //    {
    //        if (error)
    //            *error = kBaseErr_BlocksDoors;
    //        return 0;
    //    }
    //    if (!roomDetailsReachable(room,&fill_state,NULL))
    //    {
    //        if (error)
    //            *error = kBaseErr_BlocksDetail;
    //        return 0;
    //    }
    //}
