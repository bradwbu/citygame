...
void SubtractFromMaxAbsorbMods(Character *p, float amount)
{
    ...
    while (amount > 0.0f && absorbMod)
    {
        if (absorbMod->ptemplate->offAspect == offsetof(CharacterAttribSet, pattrMax)
            && absorbMod->ptemplate->offAttrib == offsetof(CharacterAttributes, fAbsorb))
        {
...
void character_AccrueBoosts(Character *p, Power *ppow, float fRate)
{
    ...
    if(ppow->ppowBase->bIgnoreStrength)
    {
            ...
            i--)
        {
            ...
            if(ptemplate->offAttrib < sizeof(CharacterAttributes))
            {
                float *pf = ATTRIB_GET_PTR(ppow->pattrStrength, ptemplate->offAttrib);
                *pf = 1.0f;
            }
        }
    }
    ...
    if(!p->bIgnoreBoosts)
    {
        for(i=eaSize(&ppow->ppBoosts)-1; i>=0; i--)
        {
            if(ppow->ppBoosts[i]!=NULL && ppow->ppBoosts[i]->ppowBase!=NULL)
            {
                ...
                if(fEff > 0.0f)
                {
                    ...
                    for(j=eaSize(&ppow->ppBoosts[i]->ppowBase->ppTemplates)-1; j>=0; j--)
                    {
                        float fMagnitude;
                        ...
                        if(ptemplate->bBoostTemplate)
                        {
                            ...
                            if(ptemplate->offAttrib!=kSpecialAttrib_PowerChanceMod)
                            {
                                ...
                                //   so it's been optimized down to its bare essentials.
                                {
                                    ...
                                    {
                                        pf = ATTRIB_GET_PTR(&ppow->attrStrength, ptemplate->offAttrib);
                                    }
                                    else
                                    {
                            ...
                            else
                            {
                                ...
                                PERFINFO_AUTO_START("mod_Fill", 1);
                                    mod_Fill(pmod, p, p, 0, ppow, ptemplate, NULL, 0.0f, 0.0f, 1.0f, 1.0f, 1.0f, ptemplate->fChance);
                                PERFINFO_AUTO_STOP();
        ...
        for(i=eaSize(&ppow->ppGlobalBoosts)-1; i>=0; i--)
        {
            if(ppow->ppGlobalBoosts[i]!=NULL && ppow->ppGlobalBoosts[i]->ppowBase!=NULL)
            {
                ...
                if(fEff > 0.0f)
                {
                    for(j=eaSize(&ppow->ppGlobalBoosts[i]->ppowBase->ppTemplates)-1; j>=0; j--)
                    {
                        float fMagnitude;
    ...
    for(i=eaSize(&ppow->ppBonuses)-1; i>=0; i--)
    {
        if(ppow->ppBonuses[i]->pBonusPower)
        {
            ...
            for(j=eaSize(&ppow->ppBonuses[i]->pBonusPower->ppTemplates)-1; j>=0; j--)
            {
                float fMagnitude;
...
void character_RemoveSleepMods(Character *p)
{
    ...
    while(pmod!=NULL)
    {
        if(pmod->ptemplate->offAttrib == offsetof(CharacterAttributes, fSleep))
        {
...
void character_HandleConditionalFXMods(Character *pchar)
{
    ...
    while(pmod!=NULL)
    {
        if(pmod->ptemplate->offAttrib<sizeof(CharacterAttributes))
        {
            float *pf = ATTRIB_GET_PTR(&pchar->attrCur, pmod->ptemplate->offAttrib);
            const char *pchConditionalFX = basepower_GetAttribModFX(pmod->ptemplate->pchConditionalFX, pmod->fx->pchConditionalFX);
            ...
            if(*pf>0.0f)
            {
                if(pchConditionalFX)
                {
                    ...
                    character_PlayMaintainedFX(pchar, eSrc->pchar,
                        pchConditionalFX,
                        pmod->uiTint,
                        0.0,
                ...
                character_AddStickyStates(pchar, pmod->ptemplate->piConditionalBits);
            }
            else
            {
                if(pchConditionalFX)
