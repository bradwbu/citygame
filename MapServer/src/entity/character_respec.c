...
#define MAX_BOOST_SLOTS 150

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
extern void RemoveStoryArc(Entity* player, StoryInfo* info, StoryArcInfo* storyarc, StoryContactInfo* contactInfo, int bClearAssignedFlag); // from storyarc.c

#define PATRON_COUNT            4
#define    PATRON_SOUVENIR_COUNT    4
#define    PATRON_STORYARCS_COUNT    4

typedef struct respecPatronData {
    char            *badgeName;
    char            *tokenName;
    char            *contactName;
    char            *souvenirSets[PATRON_SOUVENIR_COUNT];
    char            *storyarcSets[PATRON_STORYARCS_COUNT];
} respecPatronData;

respecPatronData respec_Patrons[PATRON_COUNT] =
{
    {
        "SpidersKissPatron", "GhostWidowPatron", "Ghost_Widow", 
        {
            "SL8_GhostWidow_Storyarc2", 
            "SL8_GhostWidow_Storyarc3", 
            "SL9_GhostWidow_Storyarc1",
            "SL9_GhostWidow_Storyarc3"
        },
        {    
            "V_Contacts/Grandville/tasks/SL8_GV_GhostWidow_Storyarc2.storyarc",
            "V_Contacts/Grandville/tasks/SL8_GV_GhostWidow_Storyarc3.storyarc",
            "V_Contacts/Grandville/tasks/SL9_GV_GhostWidow_Storyarc1.storyarc",
            "V_Contacts/Grandville/tasks/SL9_GV_GhostWidow_Storyarc3.storyarc"
        }
    },
    {
        "MiragePatron", "SciroccoPatron", "Scirocco", 
        {
            "SL8_Scirocco_StoryArc1_Souvenir", 
            "SL8_Scirocco_StoryArc2_Souvenir", 
            "SL9_Scirocco_StoryArc1_Souvenir",
            "SL9_GV_Scirocco_StoryArc2_Souvenir"
        },
        {    
            "V_Contacts/Grandville/tasks/SL8_GV_Scirocco_StoryArc1.storyarc",
            "V_Contacts/Grandville/tasks/SL8_GV_Scirocco_StoryArc2.storyarc",
            "V_Contacts/Grandville/tasks/SL9_GV_Scirocco_StoryArc2.storyarc",
            "V_Contacts/Grandville/tasks/SL9_GV_Scirocco_StoryArc1.storyarc"
        }
    },
    {
        "BloodInTheWaterPatron", "CaptainMakoPatron", "Captain_Mako", 
        {
            "SL8_GV_Mako_Story_Arc_1_Souvenir", 
            "SL8_GV_Mako_Story_Arc_2_Souvenir",
            "SL9_GV_Mako_Story_Arc_1_Souvenir",
            "SL9_GV_Mako_Story_Arc_2_Souvenir" 
        },
        {    
            "V_Contacts/Grandville/tasks/SL8_GV_Mako_Story_Arc_1.storyarc",
            "V_Contacts/Grandville/tasks/SL8_GV_Mako_Story_Arc_3.storyarc",
            "V_Contacts/Grandville/tasks/SL9_GV_Mako_Story_Arc_2.storyarc",
            "V_Contacts/Grandville/tasks/SL9_GV_Mako_Story_Arc_1.storyarc"
        }
    },
    {
        "TheStingerPatron", "BlackScorpionPatron", "Black_Scorpion", 
        {
            "SL8_GV_Black_Scorpion_Story_Arc_1_Souvenir", 
            "SL8_GV_Black_Scorpion_Story_Arc_2_Souvenir", 
            "SL9_BlackScorpion_StoryArc1_Souvenir",
            "SL9_GV_BlackScorpion_Storyarc2_SouvenirClue"
        },
        {    
            "V_Contacts/Grandville/tasks/SL8_GV_BlackScorpion_Story_Arc_1.storyarc",
            "V_Contacts/Grandville/tasks/SL8_GV_BlackScorpion_Story_Arc_3.storyarc",
            "V_Contacts/Grandville/tasks/SL9_GV_BlackScorpion_StoryArc2.storyarc",
            "V_Contacts/Grandville/tasks/SL9_GV_BlackScorpion_StoryArc1.storyarc"
        }
    },
};
...
void respecSwapPatronBadgesAndTokens(Entity *e, int newPatron, int oldPatron)
{
    RewardToken *pToken = NULL;

    if (newPatron >= 0 && oldPatron >= 0 && newPatron != oldPatron)
    {
        removeRewardToken(e, respec_Patrons[oldPatron].tokenName);
        badge_Revoke(e, respec_Patrons[oldPatron].badgeName);

        if ((pToken = getRewardToken(e, respec_Patrons[newPatron].tokenName)) == NULL) 
        {
            pToken = giveRewardToken(e, respec_Patrons[newPatron].tokenName);
        }        
        if (pToken)
        {
            pToken->val = 1;
        ...
        badge_Award(e, respec_Patrons[newPatron].badgeName, false);
    }
}

void respecChangePatron(Entity *e, int newPatron, int oldPatron)
{
    int i;
    ...
    if (newPatron >= 0 && oldPatron >= 0 && newPatron != oldPatron)
    {
        ...
        StoryInfo* info = NULL;
        ContactHandle contact = ContactGetHandleLoose(respec_Patrons[oldPatron].contactName);
        ContactHandle newcontact = 0;
        int k, newstoryArcCount;
        StoryContactInfo *pOldContact = NULL;
        StoryContactInfo *pNewContact = NULL;


        // add new contact
        ContactDebugAdd(NULL, e, respec_Patrons[newPatron].contactName);

        // Set the first StoryArc as given, since a player must have completed that one in order to have a patron.  Allow the player to complete
        //        the rest for the new patron.
        newcontact = ContactGetHandleLoose(respec_Patrons[newPatron].contactName); // has to happen after this contact is added to player

        info = StoryArcLockPlayerInfo(e);
        pOldContact = ContactGetInfo(info, contact);
        pNewContact = ContactGetInfo(info, newcontact);
        newstoryArcCount = eaSize(&pNewContact->contact->def->storyarcrefs);
        for (k = 0; k < newstoryArcCount; k++)
        {
            if (stricmp(pNewContact->contact->def->storyarcrefs[k]->filename, respec_Patrons[newPatron].storyarcSets[0]) == 0)
            {
                BitFieldSet(pNewContact->storyArcIssued, STORYARC_BITFIELD_SIZE, k, 1);
                break;
            }
        }
        StoryArcUnlockPlayerInfo(e);

        // remove old contact
        if (contact != 0)
        {
            ContactDismiss(e, contact, 0);
        }

        // modify souvenier clues.
        for (i = 0; i < PATRON_SOUVENIR_COUNT; i++)
        {
            const SouvenirClue* clue = scFindByName(respec_Patrons[oldPatron].souvenirSets[i]);
            if(clue)
            {
                int index = eaFind(&e->storyInfo->souvenirClues, clue);
                // Does the player already have this clue?
                if (index >= 0)
                {
                    eaRemoveConst(&e->storyInfo->souvenirClues, index);
                    scApplyToEnt(e, (const char**)eaFromPointerUnsafe(respec_Patrons[newPatron].souvenirSets[i]));
        ...
        LOG_ENT( e, LOG_ENTITY, LOG_LEVEL_IMPORTANT, 0, "Respec:PatronChange Player has changed patron to %s", respec_Patrons[newPatron].contactName);
...
static bool respec_VerifyBoosts(Entity * e, Character * tchar, Character * pCharOld, RespecBoost*** pppCurrentBoosts, RespecBoost*** pppRespecBoosts, int iExpLevel, BasePower* aBoostSlots[MAX_BOOST_SLOTS], int iSlotCount, int * piPileValue)
{
        ...
        for( j = eaSize(&tchar->ppPowerSets)-1; j > 0; j-- ) // intentionally > as opposed to >=
        {
            for( k = eaSize(&tchar->ppPowerSets[j]->ppPowers)-1; k >= 0; k-- )
            {
                if( !bFoundPower && aBoostSlots[i] == tchar->ppPowerSets[j]->ppPowers[k]->ppowBase )
                {
                    ...
                    // see if character was high enough level to have bought the slot at this point
                    if (ppow->iLevelBought > tchar->iLevel)
                    {
                        LOG_ENT( e, LOG_ENTITY, LOG_LEVEL_VERBOSE, 0, "Respec Attempted to buy slot for power %s, must be level %d (current level is %d)", ppow->ppowBase->pchName, ppow->iLevelBought, tchar->iLevel);
                        return false;
                    }
    ...
    for( i = 0; i < iSize; i++ )
    {
        ...
        if( prb->bToPile ) // if its going to pile find it in original list, remove it and pay for it
        {
            const StoreItem *psi = store_FindItem(pBoost->ppowBase, (pBoost->iLevel + pBoost->iNumCombines));
            if(psi)
            {
                (*piPileValue)  += psi->iSell;
                LOG_ENT( e, LOG_ENTITY, LOG_LEVEL_IMPORTANT, 0, "Respec Sold boost %s for %d\n", pBoost->ppowBase->pchName, psi->iSell);
            }
            else
            {
                LOG_ENT( e, LOG_ENTITY, LOG_LEVEL_IMPORTANT, 0, "Respec Unable to find price to sell boost %s. Player gets nothing.\n", pBoost->ppowBase->pchName);
            }
        }
            ...
            for( j = eaSize(&tchar->ppPowerSets)-1; j >= 0; j-- )
            {
                for( k = eaSize(&tchar->ppPowerSets[j]->ppPowers)-1; k >= 0; k-- )
                {
                    if( !bFoundPower && prb->pToPow == tchar->ppPowerSets[j]->ppPowers[k]->ppowBase )
                    {
                        ...
                        if (boost_InventionLicenseLocked(pBoost, e))
                        {
                            LOG_ENT( e, LOG_ENTITY, LOG_LEVEL_DEBUG, 0, "Respec license locked boost for power or character (%s in power %s)\n", pBoost->ppowBase->pchName, tchar->ppPowerSets[j]->ppPowers[k]->ppowBase->pchName);
                            return false;
                        }
                               ...
                               RespecBoost ***pppCurrentBoosts, const BasePower* aBoostSlots[MAX_BOOST_SLOTS], int * piSlotCount, int *patronID)
{
    ...
    if (patronID != NULL)
    {
        *patronID = iPatronID;
        ...
        // do badges and tokens so powers validate correctly
        if (iPatronID >= 0)
        {
            respecSwapPatronBadgesAndTokens(e, iPatronID, respecFindPatron(e));
        }
    }
...
bool character_ReceiveRespec( Packet * pak, Entity * e )
{
    ...
    if( bSuccess )
    {
        ...
        respecChangePatron(e, newPatronID, oldPatronID);
    ...
    else
    {
        ...
        // undo any patron changes
        respecSwapPatronBadgesAndTokens(e, oldPatronID, newPatronID);
