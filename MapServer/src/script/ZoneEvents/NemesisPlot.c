...
static void NemesisPlotSpawnTroops()
{
    ...
    NUMBER spawnOnAccesslevel = VarGetNumber("SpawnTroopsOnAccesslevel");
    char buf[2000];
    ...
    if (currentTroops < VarGetNumber("MaxTroopsPerWave") && currentTroops < (numPlayers * 3))
    {
        ...
        if (numPlayers > 0)
        {
            ...
            while (loop < count)
            {
                ...
                accessCheck = spawnOnAccesslevel || (GetAccessLevel(target) <= 0);

                if (!IsEntityMoving(target) && 
...
void NemesisPlotInit()
{
    ...
    SetScriptVar( "SpawnTroopsOnAccesslevel",                "0",                        SP_OPTIONAL );
