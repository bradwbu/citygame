...
void InfoBox(int msg_type, char *text, ...) {
    ...
    if (g_verbose_client)
    {
        sendChatToLauncher("InfoBox:%d:%s\n",msg_type,text);
        printf("[info]: %s\n", text);
    }
...
static float getAttackRating(const BasePower *base) {
    ...
    if (base->eTargetType == kTargetType_Villain ||
        ...
        for( i = 0; i < eaSize( &base->ppTemplates ); i++ )
        {
            const AttribModTemplate *mod = base->ppTemplates[i];
            if (IS_HITPOINTS(mod->offAttrib)) {
                ret += mod->fMagnitude;
            }
        }
    }
...
void addToIgnoreList(Entity *e) {
    if (!(g_testMode & TEST_MOVE) || (g_testMode & TEST_NOIGNORE))
        return; // Never add to ignore list when we're not moving!
