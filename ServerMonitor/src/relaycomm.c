...
void receiveStatusFromClient(Packet * pak, NetLink * link)
{
    ...
    if(con->protocol == CMDRELAY_PROTOCOL_VERSION)
    {
        ...
        con->arenaserverCount    = pktGetBitsPack(pak, 1);
        con->crashedArenaCount    = pktGetBitsPack(pak, 1);

        strcpy(con->version,    pktGetString(pak));
        strcpy(con->lastMsg,    pktGetString(pak));
        con->logserverCount        = pktGetBitsPack(pak, 1);

        // now set text versions
        strcpy(con->typeStr, StaticDefineIntRevLookup(cmdRelayTypeDef, con->type));
